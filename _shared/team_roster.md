...
> **Pre-seeded with an illustrative 4-agent structure (Diana + Tom + Sarah + Bob).** This file tells the system who's on your team, who owns what kinds of work, and what each agent should elevate to you instead of handling themselves. Diana edits it in place to match the real team — names, tenure, elevation rules. Two ways: (1) edit the sections below by hand, replacing the illustrative agents with your actual team, or (2) paste the interview prompt at the bottom of this file into a chat and let it produce a finished roster. The structure (Sign-offs / Elevates / Handles independently) is the convention all roster entries follow. To start from a blank slate instead of editing here, use `_shared/team_roster_TEMPLATE.md`. **No client names or deal details — roles, rules, and routing only.**
